 ...
 */
export async function getCardsForPage(
  filterParam: i.Filter,
  page: number,
  pageSize: number
): Promise<{ cards: i.Card[]; totalPages: number }> {
  ...
  if (filterParam.colorlessManaChecked === "false")
    includeManaConditions.push("\\d+", "X");
  else {
  ...
  if (includeManaConditions.length > 0) {
    query.manaCost = {
      $regex: new RegExp(
        `^(${includeManaConditions.map((c) => `\\{${c}\\}`).join("|")})+$`,
        "i"
      ),
    };
  }
  ...
  if (
    filterParam.sort &&
    filterParam.sortDirection &&
    filterParam.sort != "" &&
    filterParam.sortDirection != ""
  ) {
    sortOptions[filterParam.sort] =
      filterParam.sortDirection === "down" ? 1 : -1;
  }

  const skipDocuments: number = (page - 1) * pageSize;
  const cards: i.Card[] = await db.cardsCollection
    .find(query)
    .sort(sortOptions)
    .skip(skipDocuments)
    .limit(pageSize)
  ...
  const totalPages = getTotalPages(
    await db.cardsCollection.countDocuments(query),
    pageSize
  );
  return {
    cards: cards,
...
export function getCardsForPageFromArray(
  arr: i.Card[],
  page: number,
  pagesize: number
): i.Card[] {
...
export function getCardWAmauntForPage(
  allItems: Map<i.Card, number>,
  page: number,
  pageSize: number
): Map<i.Card, number> {
  const startIndex = (page - 1) * pageSize;
...
function filterManaType(
  ...
  if (manaReqQuery != undefined && manaReqQuery != "") {
    if (manaReqQuery == "false") {
      arrToFilter = arrToFilter.filter(
        (e) => e.manaCost && !e.manaCost.includes(colorCode)
      );
    }
  }
  return arrToFilter;
 ...
 * @returns filtered array
 */
function filterColorlessManaType(
  arrToFilter: i.Card[],
  manaReqQuery: any
): i.Card[] {
  if (manaReqQuery != undefined && manaReqQuery != "") {
...
export function sortBy(a: i.Card, b: i.Card, sortParam: string): number {
  if (
    typeof a[`${sortParam}`] === "string" &&
    typeof b[`${sortParam}`] === "string"
  ) {
 ...
 */
export function getDecksForPage(
  allItems: i.Deck[],
  page: number,
  pageSize: number
): i.Deck[] {
...
export function filterAndSortCards(
  ...
  if (cardLookup != undefined && cardLookup != "") {
    ...
    filteredCards = filteredCards.filter((e) =>
      `${e.name}${e.id}${e.multiverseid}`
        .toLowerCase()
        .includes(`${cardLookup}`.toLowerCase())
    );
  }
  ...
  if (
    ...
    if (`${sortDirection}` === "down") {
      sortedCards = [
        ...sortedCards.sort((a: i.Card, b: i.Card) => sortBy(a, b, `${sort}`)),
      ];
    } else {
      sortedCards = [
        ...sortedCards.sort(
          (a: i.Card, b: i.Card) => sortBy(a, b, `${sort}`) * -1
        ),
      ];
    }
  }
...
export function getChance(
  cards: i.Card[],
  card: i.Card
): { chance: number; amount: number } {
  let count = 0;
  cards.forEach((arrCard, index) => {
    if (
      arrCard &&
      typeof arrCard === typeof card &&
      arrCard.name === card.name
    ) {
      count++;
    }
  });
